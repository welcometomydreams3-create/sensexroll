...
#
# UPDATE: Flattrade tsym format is SENSEXYYMDDStrikePE|CE
# - YY is two-digit year
# - M is the calendar month number: Jan=1, Feb=2, ..., Nov=11, Dec=12
# - DD is day of month zero-padded to two digits (01..31)
# - Strike is placed before the option suffix
# Examples:
#   01-Jan-26, 87000CE -> SENSEX2610187000CE   (YY=26, M=1,  DD=01)
#   08-Jan-26, 82100CE -> SENSEX261082100CE    (YY=26, M=1,  DD=08)
#   14-Jan-26, 83600PE -> SENSEX2611483600PE   (YY=26, M=1,  DD=14)
...
STRATEGIES = [
    (20.0, "unused://flattrade"),
]

# Symbols, expiry and quantities
SYMBOL = "SENSEX"
EXPIRY_DATE = "2025-12-24"   # ISO yyyy-mm-dd (Fallback if auto-pick fails)
STRADDLE_LOTS = 5            # lots to trade per entry (per leg)
LOT_SIZE_MAP = {"SENSEX": 20} # units per lot

# Time windows
START_TIME_STR = "10:45"     # HH:MM 24h
EXIT_TIME_STR  = "15:25"     # HH:MM 24h
...
FLATTRADE_EXCHANGE = "BFO"                  # SENSEX options
...
def get_logfile_path():
    ...
    try:
        # Use SELECTED_EXPIRY if set, else EXPIRY_DATE
        target_expiry = SELECTED_EXPIRY if 'SELECTED_EXPIRY' in globals() else EXPIRY_DATE
        expiry_dd = datetime.datetime.strptime(target_expiry, "%Y-%m-%d").strftime("%d")
    except Exception:
        expiry_dd = "XX"
...
# CSV logic removed, relying on regex/date derivation
# =====================
...
def _month_code_numeric(month: int) -> str:
    """
    Return calendar month number for BFO: Jan=1, Feb=2, ..., Nov=11, Dec=12.
    """
    try:
...
def tsym_from_internal(instrument: str) -> str:
    """
    Convert internal instrument (e.g., SENSEXyymmddC26150) to Flattrade tsym:
    SENSEXYYMDDStrikeCE|PE
      - YY: 2-digit year
      - M: calendar month number (1..12, no leading zero)
      - DD: day of month, zero-padded to two digits (01..31)
      - Strike: before suffix
    Examples:
      yymmdd=260101, C 87000 -> SENSEX2610187000CE
      yymmdd=261108, P 90700 -> SENSEX2611590700PE
      yymmdd=261008, C 76700 -> SENSEX2610876700CE
    """
    ...
    try:
        ...
        # fallback to EXPIRY_DATE if parsing yymmdd fails
        try:
            dt = datetime.datetime.fromisoformat(EXPIRY_DATE)
        except Exception:
            dt = datetime.datetime.utcnow()

    yy = dt.strftime("%y")                # e.g., '26'
    dd = f"{dt.day:02d}"                  # '01', '08', '14', etc.
    mcode = _month_code_numeric(dt.month) # '1'..'12'
    opt_suffix = "CE" if opt.upper() == "C" else "PE"
    try:
        strike_int = int(strike)
    except Exception:
        strike_int = strike

    # Build new format: <SYM><YY><M><DD><STRIKE><CE|PE>
    tsym = f"{sym}{yy}{mcode}{dd}{strike_int}{opt_suffix}"
    return tsym
...
def upstox_refresh_access_token() -> Optional[str]:
    ...
    """
    # Logic simplified as REFRESH_TOKEN is empty in user config
    if not (UPSTOX_CLIENT_ID and UPSTOX_CLIENT_SECRET):
        print(f"[{nowstr()}] ⚠️ Upstox refresh skipped: missing client/secret")
        return None
    # Implementation omitted for brevity as per original request, logic intact if needed
    return None
...
def _detect_option_type_from_tsym(ts: str) -> Optional[str]:
    """
    Return "C" or "P" if tsym indicates call/put. Supports both legacy and new formats:
      - Legacy examples matched:
        SENSEX04NOV25C25750  -> "C"
        SENSEX04NOV25CE25750 -> "C"
        SENSEX04NOV25PE25750 -> "P"
        SENSEX04NOV25P25750  -> "P"
      - New format examples:
        SENSEX2610187000CE   -> "C"
        SENSEX2611590700PE   -> "P"
    """
    if not ts:
        return None
    tsu = ts.upper().strip()
    # New format ends with CE/PE
    if tsu.endswith("CE"):
        return "C"
    if tsu.endswith("PE"):
        return "P"
    # Legacy forms: CE/PE or C/P followed by digits
    m = re.search(r'(CE|PE|C|P)(\d{3,7})$', tsu, flags=re.IGNORECASE)
    if not m:
        return None
    op = m.group(1).upper()
    if op.startswith('C'):
...
def verify_entry_and_squareoff_if_needed(ctx: "StrategyContext", ce_internal: str, pe_internal: str, logfile_path: str, wait_seconds: float = 1.0) -> bool:
    ...
    try:
        ...
        # expected absolute units per leg (negative for short)
        ...
        if net_ce != expected_units or net_pe != expected_units:
            ...
            for instr in list(ctx.positions.keys()):
                try:
                    ...
                    err = f"[{nowstr()}] [PCT={ctx.trigger_pct:g}%] ⚠️ Emergency exit failed for {instr}: {e}"
                    ctx.log(err)
                    ctx.log_to_file(logfile_path, err)
            ...
            fin = f"[{nowstr()}] [PCT={ctx.trigger_pct:g}%] ℹ️ Context terminated for the day after entry mismatch."
            ctx.log(fin)
            ctx.log_to_file(logfile_path, fin)
            return False

        return True

    except Exception as ex:
        err = f"[{nowstr()}] [PCT={ctx.trigger_pct:g}%] ⚠️ Exception during entry verification: {ex}"
        ctx.log(err)
        ctx.log_to_file(logfile_path, err)
        return False
